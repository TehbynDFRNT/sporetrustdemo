...
// status, customer_id }` so the caller can build the customer-facing URL and
// fire lifecycle comms without a second lookup.
export async function ensureInspectionFromBooking(input) {
  ...
  const { data: existing, error: existingErr } = await supabase
    .from("inspections")
    .select("inspection_id, report_slug, status, customer_id")
