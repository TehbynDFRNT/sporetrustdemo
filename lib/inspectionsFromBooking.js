...
export async function ensureInspectionFromBooking(input) {
  ...
  // 3. Property — unique on (LOWER(address_line), postcode).
  const address = String(input.address || "").trim();
  const postcode = String(input.postcode || "").trim();
  if (!address) throw new Error("address is required");
  if (!postcode) throw new Error("postcode is required");
  let propertyId;
  const { data: foundProperty, error: pLookupErr } = await supabase
    .from("properties")
    .select("property_id")
    .ilike("address_line", address)
    .eq("postcode", postcode)
    .maybeSingle();
  if (pLookupErr) throw new Error(`Lookup property: ${pLookupErr.message}`);
  if (foundProperty) {
    propertyId = foundProperty.property_id;
  } else {
    const { data: newProperty, error: pInsertErr } = await supabase
      .from("properties")
      .insert({
      ...
      .select("property_id")
      .single();
    if (pInsertErr) throw new Error(`Insert property: ${pInsertErr.message}`);
