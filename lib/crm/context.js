...
export async function buildCardContext(supabase, cardId) {
  ...
  const context = {
    ...
    business: {
      ...
      notes:
        "In waitlist mode there are NO bookable inspection slots right now — demand exceeds current capacity, so qualified leads join a priority waitlist and must never be promised a booking or a date. Calls are the highest-value action. Never suggest more than one outbound message within 48h of the last outbound touch.",
    },
    customer: {
