...
// Status-driven kanban — one column per pipeline state. Data is fetched once
// at the Inspections page level (queryKey ["admin-table","inspections"]) and
// passed in as props; this view is pure presentation.
//
// Pipeline buckets (in priority order):
//   1. Active            — started_at set, completed_at null
//   2. Upcoming          — status=scheduled (past or future)
//   3. Awaiting sign-off — completed_at set, signed_off_at null, report draft
//   4. Recent            — published / signed (last 10, completed_at desc)
//   5. Cancelled / no-show — surfaced last
export default function PipelineView({ rows, isLoading, isError, error }) {
  const active = rows
